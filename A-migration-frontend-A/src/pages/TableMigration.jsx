...
const TableMigration = () => {
  ...
  const fetchSchemaObjects = async (schema) => {
    try {
      const res = await fetch(`https://backend-open-db.apps.itz-47ubpb.infra01-lb.dal14.techzone.ibm.com/${contextType}?schema=${schema}&source=${sourceDbType}`);
  ...
  const handleStartMigration = () => {
    ...
    const url = `https://backend-open-db.apps.itz-47ubpb.infra01-lb.dal14.techzone.ibm.com/migrate-tables/${sourceDbType}/${selectedSchema}/stream?${queryParams}&include_empty=true&transaction_id=${transactionId}`;
  ...
  useEffect(() => {
    ...
    if (showStatusModal && transactionId) {
      intervalId = setInterval(async () => {
        try {
          const res = await fetch(`https://backend-open-db.apps.itz-47ubpb.infra01-lb.dal14.techzone.ibm.com/migration-status/${transactionId}`);
          if (res.ok) {
            const data = await res.json();
