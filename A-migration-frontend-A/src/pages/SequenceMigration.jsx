...
const SequenceMigration = () => {
  ...
  const fetchSequences = async (schema) => {
    ...
    try {
      ...
      const res = await fetch(`https://backend-open-db.apps.itz-47ubpb.infra01-lb.dal14.techzone.ibm.com/sequences${apiPrefix}/${schema}/list`);
  ...
  const startMigration = () => {
    ...
    const url = `https://backend-open-db.apps.itz-47ubpb.infra01-lb.dal14.techzone.ibm.com/sequences/migrate/stream?source_type=${sourceDbType}&schema=${selectedSchema}&transaction_id=${transactionId}`;
